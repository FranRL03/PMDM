...
export class SimpsonListComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.simpsonService.getSimpsonList().subscribe(resp => this.simpsonList = resp.docs);
    }

    open(modal: any, id: string) {
        this.simpsonService.getSimpsonId(id).subscribe(resp => this.simpsonSelected = resp._id);
